# 🔧 故障排查指南

## 问题：点击"AI 生成细纲"没有反应

### ✅ 已修复

**问题原因：**
- `generateChapterBeats` 函数只支持 Google Gemini 提供商
- 使用其他提供商（DeepSeek、OpenAI、Custom）时会返回空数组
- 没有错误提示，导致用户不知道发生了什么

**修复内容：**
1. ✅ 添加了对所有提供商的支持
2. ✅ 添加了 Token 预算检查
3. ✅ 添加了使用记录
4. ✅ 改进了错误处理

**现在支持：**
- ✅ Google Gemini
- ✅ DeepSeek
- ✅ OpenAI
- ✅ 自定义提供商

---

## 常见问题排查

### 1. 按钮点击无反应

**可能原因：**
- [ ] 未配置 API Key
- [ ] 章节信息不完整
- [ ] Token 预算已用完
- [ ] 网络连接问题
- [ ] API 调用失败

**排查步骤：**

1. **检查 API Key**
   ```
   应用设置 → AI 模型服务商 → 检查 API Key 是否已填写
   ```

2. **检查章节信息**
   - 章节标题是否已填写
   - 章节摘要是否已填写

3. **查看浏览器控制台**
   - 按 F12 打开开发者工具
   - 切换到 Console 标签
   - 查看是否有错误信息

4. **检查 Token 预算**
   ```
   应用设置 → Token 使用统计 → 查看今日使用量
   ```

5. **测试网络连接**
   - 尝试生成其他内容（如章节内容）
   - 检查是否能访问 AI 提供商的 API

---

### 2. 生成的细纲质量不好

**可能原因：**
- 章节摘要写得太简单
- 缺少上下文信息
- 模型选择不当

**解决方法：**

1. **优化章节摘要**
   - 写得更详细具体
   - 包含关键情节点
   - 提及重要角色

2. **完善项目设置**
   - 填写详细的世界设定
   - 添加主要角色
   - 设置明确的主线剧情

3. **选择更好的模型**
   - Google: `gemini-2.0-flash-exp` 或 `gemini-1.5-pro`
   - OpenAI: `gpt-4o`
   - DeepSeek: `deepseek-chat`

---

### 3. 生成速度很慢

**可能原因：**
- 网络延迟
- API 服务器负载高
- 模型响应慢

**解决方法：**

1. **检查网络**
   - 使用网络测速工具
   - 尝试切换网络

2. **更换模型**
   - 使用 Flash 系列模型（更快）
   - Google: `gemini-2.0-flash-exp`
   - OpenAI: `gpt-4o-mini`

3. **使用本地模型**
   - 安装 Ollama
   - 使用本地 LLM

---

### 4. 提示 "Token budget exceeded"

**原因：** 超出每日 Token 限制

**解决方法：**

1. **等待重置**
   - Token 预算每天 00:00 重置

2. **增加限制**
   ```
   应用设置 → 高级设置 → Token 预算控制 → 增加每日限制
   ```

3. **临时禁用预算**
   ```
   应用设置 → 高级设置 → Token 预算控制 → 关闭开关
   ```

---

### 5. API 调用失败

**错误示例：**
```
Error: OpenAI/DeepSeek API Error: 401 Unauthorized
Error: Failed to generate beats
```

**排查步骤：**

1. **检查 API Key**
   - 确认 Key 复制完整
   - 确认 Key 未过期
   - 确认 Key 有相应权限

2. **检查 Base URL**
   - Google: 留空或使用代理地址
   - DeepSeek: `https://api.deepseek.com`
   - OpenAI: `https://api.openai.com/v1`
   - Ollama: `http://localhost:11434/v1`

3. **检查账户余额**
   - OpenAI: 查看账户余额
   - DeepSeek: 查看账户余额

4. **测试 API 连接**
   ```bash
   # Google Gemini
   curl https://generativelanguage.googleapis.com/v1beta/models \
     -H "x-goog-api-key: YOUR_API_KEY"
   
   # OpenAI
   curl https://api.openai.com/v1/models \
     -H "Authorization: Bearer YOUR_API_KEY"
   
   # DeepSeek
   curl https://api.deepseek.com/v1/models \
     -H "Authorization: Bearer YOUR_API_KEY"
   ```

---

### 6. 中国大陆访问问题

**症状：**
- 请求超时
- 连接失败
- 403 Forbidden

**解决方法：**

1. **使用代理**
   ```typescript
   // 在应用设置中配置
   {
     provider: 'google',
     apiKey: 'YOUR_KEY',
     baseUrl: 'https://your-proxy.com/v1beta'
   }
   ```

2. **使用国内可用的提供商**
   - DeepSeek（国内可直接访问）
   - 阿里云通义千问
   - 百度文心一言

3. **使用本地模型**
   - Ollama + Llama 3
   - 完全本地运行，无需网络

---

## 调试技巧

### 1. 查看详细日志

**浏览器控制台：**
```javascript
// 查看 Token 使用情况
import { tokenCounter } from './services/tokenCounter';
console.log(tokenCounter.getStats());

// 查看向量数据
const request = indexedDB.open('InkFlowDB', 5);
request.onsuccess = () => {
    const db = request.result;
    const tx = db.transaction(['vectors'], 'readonly');
    const store = tx.objectStore('vectors');
    const req = store.getAll();
    req.onsuccess = () => console.log('Vectors:', req.result);
};
```

### 2. 测试 API 调用

**在浏览器控制台执行：**
```javascript
// 测试细纲生成
const testBeats = async () => {
    const { generateChapterBeats } = await import('./services/geminiService');
    const chapter = {
        id: 'test',
        title: '测试章节',
        summary: '主角遇到了强大的敌人',
        order: 1,
        content: '',
        wordCount: 0,
        volumeId: null,
        hooks: []
    };
    const allChapters = []; // 所有章节，用于查找上一章和祖先
    const volumes = [];     // 所有分卷，用于注入分卷上下文
    const config = {
        title: '测试小说',
        genre: '玄幻',
        // ... 其他配置
    };
    const characters = [];  // 角色列表
    const settings = {
        provider: 'google',
        apiKey: 'YOUR_KEY',
        model: 'gemini-2.0-flash-exp'
    };
    
    try {
        // 新签名: generateChapterBeats(chapter, allChapters, volumes, config, characters, settings)
        const beats = await generateChapterBeats(chapter, allChapters, volumes, config, characters, settings);
        console.log('生成的细纲:', beats);
    } catch (e) {
        console.error('错误:', e);
    }
};

testBeats();
```

### 3. 清除缓存

**如果遇到奇怪的问题：**

1. **清除浏览器缓存**
   - Chrome: Ctrl+Shift+Delete
   - 选择"缓存的图片和文件"
   - 点击"清除数据"

2. **清除 IndexedDB**
   ```javascript
   // 在浏览器控制台执行
   indexedDB.deleteDatabase('InkFlowDB');
   location.reload();
   ```

3. **重启开发服务器**
   ```bash
   # 停止服务器（Ctrl+C）
   # 重新启动
   npm run dev
   ```

---

## 性能优化建议

### 1. 减少 API 调用

- 先手动编写细纲，再让 AI 优化
- 批量生成多个章节的细纲
- 复用相似章节的细纲

### 2. 使用缓存

- 相同的章节摘要会生成相似的细纲
- 可以保存常用的细纲模板

### 3. 选择合适的模型

| 场景 | 推荐模型 | 原因 |
|------|---------|------|
| 快速草稿 | gemini-2.0-flash-exp | 速度快，免费 |
| 高质量 | gemini-1.5-pro | 质量好 |
| 成本优先 | deepseek-chat | 便宜 |
| 隐私优先 | Ollama (本地) | 完全本地 |

---

## 获取帮助

如果以上方法都无法解决问题：

1. **查看完整错误信息**
   - 打开浏览器控制台（F12）
   - 复制完整的错误堆栈

2. **提供环境信息**
   - 操作系统
   - 浏览器版本
   - AI 提供商和模型
   - 是否启用了 RAG/Token 预算

3. **提交 Issue**
   - 附上错误截图
   - 附上控制台日志
   - 描述复现步骤

---

## 更新日志

### 2024-01-XX - 细纲生成修复

- ✅ 修复了非 Google 提供商无法生成细纲的问题
- ✅ 添加了 Token 预算检查
- ✅ 改进了错误处理
- ✅ 添加了使用记录

---

**提示：** 大部分问题都可以通过查看浏览器控制台的错误信息来定位。遇到问题时，第一步永远是打开 F12 查看 Console！


---

## 🆕 最新修复

### 问题：生成细纲后页面空白（chapter.beats.map is not a function）

**错误信息：**
```
Uncaught TypeError: chapter.beats.map is not a function
at Editor (Editor.tsx:513:66)
```

**原因：**
- AI 返回的细纲数据格式不正确
- `chapter.beats` 不是数组类型
- 可能是 API 返回了错误格式的数据

**已修复：**
1. ✅ 添加了数组类型检查
2. ✅ 添加了错误提示
3. ✅ 添加了降级处理
4. ✅ 改进了错误处理

**如果仍然遇到问题：**

1. **刷新页面**（Ctrl+R 或 F5）
2. **清除浏览器缓存**
3. **检查 API 返回数据**
   ```javascript
   // 在浏览器控制台查看
   console.log('Beats data:', chapter.beats);
   console.log('Is array:', Array.isArray(chapter.beats));
   ```

4. **重新生成细纲**
   - 确保章节摘要完整
   - 确保 API Key 有效
   - 尝试不同的 AI 模型

---

## 📊 数据格式说明

### 正确的细纲格式

```typescript
chapter.beats = [
  "主角登场，展示实力",
  "遇到强敌，陷入困境",
  "突破境界，反败为胜",
  "获得宝物，实力提升",
  "埋下伏笔，引出下章"
]
```

### 错误的格式（会导致崩溃）

```typescript
// 不是数组
chapter.beats = "主角登场，展示实力"

// 对象格式
chapter.beats = { 0: "主角登场", 1: "遇到强敌" }

// null 或 undefined
chapter.beats = null
```

---

## 🔍 调试技巧

### 查看细纲数据

在浏览器控制台执行：

```javascript
// 获取当前章节数据
const getCurrentChapter = () => {
  // 从 React DevTools 或全局状态获取
  return window.__REACT_DEVTOOLS_GLOBAL_HOOK__.renderers.get(1)
    .getCurrentFiber().memoizedState.memoizedState;
};

// 查看细纲
console.log('Beats:', getCurrentChapter().beats);
```

### 手动修复数据

如果数据格式错误，可以手动修复：

```javascript
// 在浏览器控制台执行
const fixBeats = () => {
  const db = indexedDB.open('InkFlowDB', 5);
  db.onsuccess = () => {
    const transaction = db.result.transaction(['projects'], 'readwrite');
    const store = transaction.objectStore('projects');
    const request = store.getAll();
    
    request.onsuccess = () => {
      const projects = request.result;
      projects.forEach(project => {
        project.chapters.forEach(chapter => {
          // 确保 beats 是数组
          if (chapter.beats && !Array.isArray(chapter.beats)) {
            chapter.beats = [];
          }
        });
        store.put(project, project.id);
      });
      console.log('数据已修复');
    };
  };
};

fixBeats();
```

---

## ✅ 验证修复

修复后，应该能够：
- ✅ 正常生成细纲
- ✅ 查看和编辑细纲
- ✅ 添加和删除细纲项
- ✅ 页面不会崩溃

如果问题持续，请查看浏览器控制台的完整错误信息。
